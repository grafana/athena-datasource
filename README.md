...
| `$__unixEpochFilter(column)`         | `$__unixEpochFilter` achieves the same than $\_\_timeFilter but when the time is a UNIX timestamp.                                                                                                                                  | `$__unixEpochFilter(time)`                                    | `time BETWEEN 1637228322 AND 1637232700`                                                              |
| `$__unixEpochGroup(column, '1m')`    | `$__unixEpochGroup` achieves the same than $\_\_timeGroup but when the time is a UNIX timestamp.                                                                                                                                    | `$__unixEpochGroup(time, '5m')`                               | `FROM_UNIXTIME(FLOOR(time/300)*300)`                                                                  |
