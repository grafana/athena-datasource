...
func (c *API) getOption(options sqlds.Options, option string) string {
	v, ok := options[option]
	if !ok {
		return ""
	}
	if v == awsModels.DefaultKey {
		switch option {
		case "region":
...
func (c *API) Databases(ctx aws.Context, options sqlds.Options) ([]string, error) {
	catalog := c.getOption(options, "catalog")
	if catalog == "" {
		return nil, fmt.Errorf("missing catalog in input")
	}
	res := []string{}
	var nextToken *string
	isFinished := false
	if catalog == awsModels.DefaultKey {
...
func (c *API) Tables(ctx aws.Context, options sqlds.Options) ([]string, error) {
	catalog, database := c.getOption(options, "catalog"), c.getOption(options, "database")
	res := []string{}
	var nextToken *string
	isFinished := false
	for !isFinished {
...
func (c *API) Columns(ctx aws.Context, options sqlds.Options) ([]string, error) {
	...
	out, err := c.Client.GetTableMetadataWithContext(ctx, &athena.GetTableMetadataInput{
		CatalogName:  aws.String(catalog),
		DatabaseName: aws.String(database),
		TableName:    aws.String(options["table"]),
	})
	if err != nil {
		return nil, err
	}
